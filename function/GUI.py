...
class MapDisplay:
    def __init__(self, master):
        ...
        self.start_thread(self.map_subscriber_thread, "/map", OccupancyGrid, self.map_callback)
        self.start_thread(self.goal_subscriber_thread, "/move_base_simple/goal", PoseStamped, self.rviz_goal_callback)
        self.start_thread(self.path_subscriber_thread, "/move_base_node/SBPLLatticePlanner/plan", Path, self.path_callback)
        self.start_thread(self.front_scan_subscriber_thread, "/f_scan", LaserScan, self.f_scan_callback)
        self.start_thread(self.back_scan_subscriber_thread, "/b_scan", LaserScan, self.b_scan_callback)
        ...
        self.canvas.mpl_connect("button_press_event", self.queue_mouse_event)
        self.canvas.mpl_connect("motion_notify_event", self.queue_mouse_event)
        self.canvas.mpl_connect("button_release_event", self.queue_mouse_event)

        # Mouse event queue and thread
        self.mouse_event_queue = queue.Queue()
        self.mouse_thread = threading.Thread(target=self.process_mouse_events)
    ...
    def create_widgets(self):
        ...
        self.battery_percentage_display.grid(row=7, column=0, columnspan=2, pady=10)
    
    def start_thread(self, thread_func, topic, msg_type, callback):
        thread = threading.Thread(target=thread_func, args=(topic, msg_type, callback))
        thread.daemon = True  # Đảm bảo các luồng sẽ dừng khi chương trình kết thúc
        thread.start()

    def map_subscriber_thread(self, topic, msg_type, callback):
        rospy.Subscriber(topic, msg_type, callback)
        rospy.spin()

    def goal_subscriber_thread(self, topic, msg_type, callback):
        rospy.Subscriber(topic, msg_type, callback)
        rospy.spin()

    def path_subscriber_thread(self, topic, msg_type, callback):
        rospy.Subscriber(topic, msg_type, callback)
        rospy.spin()

    def front_scan_subscriber_thread(self, topic, msg_type, callback):
        rospy.Subscriber(topic, msg_type, callback)
        rospy.spin()

    def back_scan_subscriber_thread(self, topic, msg_type, callback):
        rospy.Subscriber(topic, msg_type, callback)
        rospy.spin()
    ...
    def queue_mouse_event(self, event):
        """Queue mouse events for processing."""
        self.mouse_event_queue.put(event)

    def process_mouse_events(self):
        """Process mouse events in a separate thread."""
        while True:
            event = self.mouse_event_queue.get()
            if event.name == "button_press_event":
                self.process_on_click(event)
            elif event.name == "motion_notify_event":
                self.process_on_drag(event)
            elif event.name == "button_release_event":
                self.process_on_release(event)

    def process_on_click(self, event):
        """Handle mouse press."""
        if event.inaxes == self.ax and self.nav_goal_mode:
            self.drag_start = (event.xdata, event.ydata)

    def process_on_drag(self, event):
        """Handle mouse drag."""
        if self.drag_start is not None and event.inaxes == self.ax:
            x_start, y_start = self.drag_start
            x_end, y_end = event.xdata, event.ydata

            self.master.after(0, lambda: self.update_drag_display(x_start, y_start, x_end, y_end))

    def process_on_release(self, event):
        """Handle mouse release."""
        if self.drag_start is not None and event.inaxes == self.ax and self.nav_goal_mode:
            x_end, y_end = event.xdata, event.ydata
            z, w = self.calculate_orientation(self.drag_start, (x_end, y_end))

            self.master.after(0, lambda: self.update_release_display(z, w))
        self.drag_start = None

    def update_drag_display(self, x_start, y_start, x_end, y_end):
    ...
    def update_release_display(self, z, w):
        """Update release visualization and send goal."""
        self.entry_z.delete(0, tk.END)
        self.entry_z.insert(0, str(z))
        self.entry_w.delete(0, tk.END)
        self.entry_w.insert(0, str(w))
