...
class MapDisplay:
    def __init__(self, master):
        ...
        self.drag_start = None  # Biến lưu vị trí bắt đầu kéo chuột
        self.path_points = []  # Danh sách đường đi

        # Subscriber cho bản đồ
        self.map_subscriber = rospy.Subscriber("/map", OccupancyGrid, self.map_callback)
        # Subscriber cho điểm đích từ RViz
        self.goal_subscriber = rospy.Subscriber("/move_base_simple/goal", PoseStamped, self.rviz_goal_callback)
        # Subscriber cho đường đi của robot
    ...
    def create_widgets(self):
        ...
        self.nav_goal_mode = False  # Biến theo dõi chế độ Nav Goal
    ...
    # Cập nhật vị trí kéo chuột
        if self.drag_start is not None and event.inaxes == self.ax:
            ...
            # Vẽ mũi tên từ điểm bắt đầu đến điểm kết thúc chỉ nếu có sự di chuyển
            if (x_end is not None and y_end is not None):
                self.ax.annotate('', xy=(x_end, y_end), xytext=(x_start, y_start),
                                arrowprops=dict(arrowstyle='->', color='red', linewidth=2))

            # Cập nhật canvas mà không chặn việc cập nhật vị trí robot
            self.canvas.draw()
