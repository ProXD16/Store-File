...
class MapDisplay(QMainWindow):
    def __init__(self):
        ...
        self.background = None  # Lưu trữ nền để sử dụng cho blitting
    ...
    def init_ui(self):
        ...
        self.send_goal_button.clicked.connect(self.send_goal_thread)
        self.control_layout.addWidget(self.send_goal_button)
    ...
    def f_scan_callback(self, msg):
        threading.Thread(target=self.update_f_points, args=(msg,)).start()

    def b_scan_callback(self, msg):
        threading.Thread(target=self.update_b_points, args=(msg,)).start()

    def update_f_points(self, msg):
        self.f_points = self.process_lidar_data(msg, False)
        self.draw_map_and_lidar()

    def update_b_points(self, msg):
    ...
    def process_lidar_data(self, msg, is_back):
        ...
        for i in range(0, len(msg.ranges), 10):  # Lấy mẫu mỗi 10 điểm
            r = msg.ranges[i]
    ...
    def draw_map_and_lidar(self):
        ...
        # Lưu nền cho lần đầu tiên
        if self.background is None:
            self.canvas.draw()
            self.background = self.canvas.copy_from_bbox(self.ax.bbox)

        # Vẽ lại các phần thay đổi
