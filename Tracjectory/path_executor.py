...
class MapDisplay:
    ...
    def create_notebook(self):
        ...
        self.notebook.grid(row=0, column=0, columnspan=2, sticky="nsew") # Đặt notebook ở trên cùng
    ...
    def create_widgets(self):
        ...
        send_button = tk.Button(self.main_tab, text="Gửi mục tiêu", command=self.send_goal_thread)
        ...
        tk.Label(speed_frame, text="Vận tốc tuyến tính:").grid(row=0, column=0)
        self.entry_linear_speed = tk.Entry(speed_frame)
        self.entry_linear_speed.insert(0, str(self.linear_speed))
        self.entry_linear_speed.grid(row=0, column=1)

        tk.Label(speed_frame, text="Vận tốc góc:").grid(row=1, column=0)
        self.entry_angular_speed = tk.Entry(speed_frame)
        self.entry_angular_speed.insert(0, str(self.angular_speed))
        self.entry_angular_speed.grid(row=1, column=1)

        move_button = tk.Button(self.main_tab, text="Chạy", command=self.move_along_path_thread)
        move_button.grid(row=4, column=0, columnspan=2, pady=5)

        stop_button = tk.Button(self.main_tab, text="Dừng", command=self.stop_movement)
        stop_button.grid(row=5, column=0, columnspan=2, pady=5)

        self.position_label = tk.Label(self.main_tab, text="Vị trí robot: (0.00, 0.00)")
        self.position_label.grid(row=6, column=0, columnspan=2)

        # Widgets for the Drawing Tab (Vẽ)
        # Frame cho vẽ đường
        line_frame = ttk.Frame(self.drawing_tab)
        line_frame.grid(row=0, column=0, columnspan=2, pady=5)

        self.draw_line_button = tk.Button(line_frame, text="Vẽ đường thẳng", command=self.toggle_draw_line_mode)
        self.draw_line_button.grid(row=0, column=0)

        self.delete_line_button = tk.Button(line_frame, text="Xóa đường thẳng", command=self.delete_line)
    ...
    def update_position(self):
        try:
            ...
            self.master.after(80, self.update_position)
    ...
    def update_lidar_points_continuously(self):
        rospy.Timer(rospy.Duration(0.08), self.update_lidar_points)
...
if __name__ == "__main__":
    ...
    # Cấu hình grid để các tab có thể mở rộng
